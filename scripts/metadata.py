...
class MetaDataItem(dict):
    ...
    def get_list(self):
        ...
        if fields == ['file']:
            return [self.file]
        elif fields == ['event_label', 'file','event_offset', 'event_onset']:
            return [self.file, self.event_onset, self.event_offset, self.event_label]
            ...
            if item.event_onset is not None and item.event_offset is not None:
                if item[self.label]:
                    ...
                    if onset <= self.event_roll.shape[0]:
                        ...
                        ####################  Added by Xianjun Xia 2017.09.09 below                        
                        '''
                        devia = 100
                        onset_new =  max(0,onset - devia)
                        offset_new = min(self.event_roll.shape[0],offset+devia)
                        i=onset_new
                        while(i<onset):
                            self.event_roll[i,pos] = 1-pow((i-onset)/devia,2)
                            i = i + 1
                        self.event_roll[onset:offset,pos] = 1
                        i = offset
                        while(i<offset_new):
                            self.event_roll[i,pos] = 1-pow((i-offset)/devia,2)
                            i = i + 1   
                        '''                                                 
                        ####################  Added by Xianjun Xia 2017.09.09 above
                        ###########  probability fixed method
                        '''
                        p = 0.0
                        onset_new  = max(0,0.5*(onset+offset) - 0.5*(offset-onset)/math.sqrt(1-p))
                        offset_new = min(self.event_roll.shape[0],0.5*(onset+offset) + 0.5*(offset-onset)/math.sqrt(1-p))
                        i=onset_new
                        while(i<offset_new): 
                            self.event_roll[i,pos] = 1-(1-p)*pow((2*i-onset-offset)/(onset-offset),2)
                            i = i + 1
                        '''
